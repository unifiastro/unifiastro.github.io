...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['csv_file'])) {
    ...
    if (($handle = fopen($file, "r")) !== FALSE) {
        ...
        while (($row = fgetcsv($handle, 0, ",", "\"", "\\")) !== FALSE) {
            ...
            $topic = htmlspecialchars($row[1] ?? 'General'); 
            $title = htmlspecialchars($row[2] ?? 'Untitled');
            $supervisor = htmlspecialchars($row[3] ?? '');
            $contacts = htmlspecialchars($row[4] ?? '');
            ...
            $type = (strpos($rawType, 'magistrale') !== false) ? 'Master' : 'Bachelor';
